...
Future<List<dynamic>?> getCultureDetailInfoRequest(String seq) async {
  const serviceKey = 'w29wQLe/8/4PUJf0wWdFZPENtTTQMK4bZD1AENMUNoy5ix8lTvxxLy7xu3y95HzLURbtwRbCPupqzu3Vj20zJw=='; // 서비스 인증키
  const url = 'http://www.culture.go.kr/openapi/rest/publicperformancedisplays/d/'; //기간별 공연/전시 정보 목록 조회 요청 url
  ...
  try {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      // perforList만 추출하여 반환
      final perforList = jsonData['response']['msgBody'][0]['perforInfo'];
      return perforList;
    } else {
      // 오류 처리
